# techinasia_scraper_playwright_v0.py

"""
File: techinasia_scraper_playwright.py
Overview: This script scrapes articles from the TechInAsia website using Playwright.
It's designed to be faster and more reliable than the Selenium version.
The scraped data is stored in a pandas DataFrame and saved in batches as CSV files.
"""
...
class TechInAsiaScraper:
    ...
    def setup_browser(self):
        ...
        try:
            ...
            self.page = self.browser.new_page(user_agent=self.config.user_agent)
    ...
    def _perform_scraping(self) -> List[Article]:
        ...
        try:
            self.page.goto(url, timeout=self.config.timeout * 1000)  # Convert seconds to milliseconds
            self.page.wait_for_selector('article.post-card', state='attached', timeout=self.config.timeout * 1000)
...
def main():
    ...
    parser.add_argument('--num_articles', type=int, help='Number of articles to scrape', default=45)
    ...
    logger.info("Starting TechInAsia scraper with Playwright")
